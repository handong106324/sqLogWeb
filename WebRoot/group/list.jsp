...
<script type="text/javascript" >
	...
	function deleteBean(id){
			if(confirm("你确定要删除该记录吗?")){
				$.ajax({
					type: "get",
					url: "${basePath}/group/delete?id=" + id,
					beforeSend: function(XMLHttpRequest){},
					success: function(data){
						if(data.result == 'success'){
							jAlert("提示信息","删除成功!",function(){document.location.href="${basePath}/group/list";});
						}else{
	...
	function setCheck(id){
			if(confirm("你确定要设置该记录为模板审核组吗?")){
				$.ajax({
					type: "get",
					url: "${basePath}/group/setCheck?id=" + id,
					beforeSend: function(XMLHttpRequest){},
					success: function(data){
						if(data.result == 'success'){
							jAlert("提示信息","设置成功!",function(){document.location.href="${basePath}/group/list";});
						}else{
							jAlert("提示信息","设置失败，请联系管理!");
						}
					},
					complete: function(XMLHttpRequest, textStatus){},
					error: function(){
						jAlert("提示信息","设置失败，请与管理员联系");
					}
				});
			}
		}
			...
			<section class="container">
				...
				<section class="area">
					...
					<section class="sub_area">
						...
						<p><a class="button white small-inp" href="${basePath }/group/showAddOrUpdate">新增+</a></p>
							...
							<tbody>
								<c:forEach items="${page.list }" var="bean">
									<tr>
										  ...
										  <td>
											  <c:forEach items="${bean.managerList}" var="gu">
												  	<div>${gu.realName}</div>
										      	</c:forEach>
									      </td>
									      <td>
									      	<a href="${basePath }/group/showAddOrUpdate/${bean.id}">修改</a>
									      	&nbsp;&nbsp;
									      	<a href="#" onclick="deleteBean('${bean.id}')">删除</a>
									      	&nbsp;&nbsp;
									      	<c:if test="${bean.isTemplateCheck !=1 }">
									      		<a href="javascript:void(0)" onclick="setCheck('${bean.id}')">设为模板审核组</a>
									      	</c:if>
									      </td>
									</tr>
								</c:forEach>
