...
function Boxy(element, options) {
    ...
    if (this.options.fixed) {
        if ($.support.msie && Query.support.version < 7) {
            this.options.fixed = false; //$.browser替换成$.support
        } else {
...
$.extend(Boxy, {
    ...
    _viewport: function() {
        ...
        return $.extend(
            $.support.msie ?//$.browser替换成$.support
...
Boxy.prototype = {
    ...
    _setupTitleBar: function() {
        if (this.options.title) {
            var self = this;
            var tb = $("<section class='title-bar'></section>").html("<h2>" + this.options.title + "</h2>");
            if (this.options.closeable) {
