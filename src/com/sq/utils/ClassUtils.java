...
//		String realClassLocation = classLocation + "\\src";
	...
	public static boolean isBelongInterface(Class<?> srcClass, Class<?> dstInterface) {
		System.out.println(srcClass.getName()+":"+dstInterface.getName());
		for (Class<?> localClass : srcClass.getInterfaces()) {
			if (localClass.equals(dstInterface))
				return true;
			if (isBelongInterface(localClass, dstInterface))
				return true;
		}
		return (srcClass.getSuperclass() != null) && (isBelongInterface(srcClass.getSuperclass(), dstInterface));
	}
	 ...
	 */
	public static String getValue(Object instance, String fieldName) throws SecurityException, NoSuchFieldException, IllegalArgumentException,
	 ...
	 * @return
	 */
	private static Field getField(Class cl, String fieldName) {
		Field f = null;
		Class clazz = cl;
	...
	public static void setObjectValue(Class cl, Object o, String key, String value) throws Exception {
		...
		if (f == null) {
			throw new Exception("类[" + cl.getName() + "]没有属性[" + key + "]，请检查！");
		}
		...
		try {
			...
			throw new Exception("类[" + cl.getName() + "]属性[" + key + "]的设置方法[" + ("set" + java.lang.Character.toUpperCase(key.charAt(0)) + key.substring(1) + "(" + t.getName() + ")") + "]，请检查！");
		}
		Object v = null;
			...
			if (value.trim().length() == 0) {
				...
				for (int i = 0; i < ss.length; i++) {
					...
					if (t == Byte.class || t == Byte.TYPE) {
						...
						throw new Exception("类[" + cl.getName() + "]属性[" + key + "]为复杂的数据类型【" + t.getName() + "的数组】，暂时不支持！");
					}
					Array.set(v, i, v2);
				}
			}
		} else {
			throw new Exception("类[" + cl.getName() + "]属性[" + key + "]为复杂的数据类型【" + t.getName() + "】，暂时不支持！");
		}
